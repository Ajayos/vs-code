...
cp -r /root/.ssh /home/BlackSudo/.ssh
chown -R coder:coder /home/BlackSudo/.ssh

# configure code-server to use --link with the "coder" user
mkdir -p /home/BlackSudo/.config/code-server
touch /home/BlackSudo/.config/code-server/config.yaml
echo "link: true" > /home/BlackSudo/.config/code-server/config.yaml
chown -R coder:coder /home/BlackSudo/.config
